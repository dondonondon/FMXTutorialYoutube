...
type
  ...
  private
    ...
    procedure fnFillData(FCount : Integer); //ctrl + shift + c
  public
...
procedure TFMain.fnFillData(FCount: Integer);
begin
  var FEvent : TPositionChangeEvent;
  var tempRecord, tempTotalRecord : Integer;

  try

    FEvent := lbProduct.OnViewportPositionChange;
    lbProduct.OnViewportPositionChange := nil;

    if lbProduct.Items.Count >= QData.RecordCount then
      Exit;
    ...
    lbProduct.OnViewportPositionChange := FEvent;
...
begin
  ...
  try
    ...
    FLoad := (Round(lbProduct.Height / (loCard.Height + 12))) * 2 + 4;
  finally
    fnIndicator(False);
    fnFillData(FLoad);
  end;
