...
type
  ...
  private
    ...
    procedure fnClickEdit(Sender : TObject); //ctrl + shift + c
  public
    FTransKelas : String;
...
uses BFA.Func, BFA.GoFrame, BFA.Helper.Control, BFA.Helper.Main, BFA.Main,
  BFA.OpenUrl, BFA.Rest, uDM, frProses;
{ TFTemp }
const
  spc = 10;
  pad = 8;
...
begin
  ...
  TCornerButton(lo.FindStyleResource('btnTempEdit')).Tag := FIndex;
  TCornerButton(lo.FindStyleResource('btnTempEdit')).OnClick := fnClickEdit;
...
procedure TFDetail.fnClickEdit(Sender: TObject);
begin
  memData.RecNo := TCornerButton(Sender).Tag;

  FProses.FJenis := 1;
  fnGoFrame(C_DETAIL, C_PROSES);
end;
...
begin
  try
    ...
    for var i := 0 to memData.RecordCount - 1 do begin
      ...
      fnLog(FNamaImage, assURL + memData.FieldByName('nip').AsString + '.png');
